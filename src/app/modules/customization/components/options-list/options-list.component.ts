...
export class MealOptionsListComponent {
  ...
  nextButton() {
    if (this.selectedStep.isTextInput) {
      // Save special instructions
      this.MealOptionsService.setSpecialInstructions(this.specialInstructionsText);
    } else {
  ...
  refreshVars() {
    ...
    if (this.selectedStep.isTextInput) {
      this.specialInstructionsText = this.MealOptionsService.specialInstructions;
    }
  }

  onSpecialInstructionsChange(text: string) {
    this.specialInstructionsText = text;
  }
}
